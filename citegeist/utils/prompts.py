# flake8: noqa
def process_data_for_extract_cited_sentences(related_work:str) -> str:
    prompt = f"""
...
def process_data_for_classify_errors(claim:str,source:str) -> str:
    prompt = f"""
    The following is a claim and its source from a related work section. Please classify the error type of the claim.
    Claim: {claim}
    Source: {source}
    
    - Error Classification Categories:
        - 1. Direct Contradiction
            The information in the claim directly contradicts explicit statements in the source text.
            Example: The claim states that a method "performs poorly with limited data," but the source clearly indicates the method is designed to "enhance capabilities with limited data." Or the claim states a method "does not address diversity issues," but the source explicitly points out its goal is to "preserve policy diversity."
        - 2. Information Not Present / Unsubstantiated
            The claim contains information that is completely absent or unsupported by the source text.
            Example: The claim mentions a method (such as RPO or HDFlow), but the provided source only discusses another method. Or the claim makes a general statement (such as "supervised fine-tuning requires large amounts of labeled data"), but the provided summary lacks corresponding support.
        - 3. Misrepresentation / Imprecise Wording
            The claim restates information from the source but uses imprecise wording or deviates from the original meaning and emphasis of the source.
            Example: The claim states that reward-maximizing reinforcement learning "over-exploits high-reward actions," while the source describes its limitations in "exploring state space and maintaining diverse hypotheses." Or the claim states a method is "still limited by predefined actions," but the source emphasizes its "contrast with static predefined actions" (implying it transcends such limitations).
        - 4. Incorrect Attribution
            The claim incorrectly attributes a contribution or characteristic described in the source to a different entity or paper.
            Example: The claim incorrectly attributes a contribution or characteristic described in the source to a different entity or paper.
            
    Please output the error type in the following format:
    {{
        "error_type": "1. Direct Contradiction",
        "error_description": "The information in the claim directly contradicts explicit statements in the source text."
    }}
    """
    return prompt

def process_data_for_correct_citation_errors(source:str,claim:str,error_type:str,error_description:str) -> str:
    prompt = f"""
You are an expert academic writing assistant. Given a list of citation errors in a related work section, your task is to provide corrected versions of the problematic claims. For each error, analyze the error type and source information, then provide a corrected claim that accurately reflects the cited works.

For each error entry, you will receive:
- error_type: The type of citation error
- error_description: Detailed explanation of the error
- claim: The original problematic claim
- source: The actual content from the cited papers

Please provide corrections following these guidelines:

1. **Direct Contradiction**: Rewrite the claim to align with what the sources actually state, removing contradictory information.

2. **Incorrect Attribution**: Ensure claims are attributed to the correct papers and accurately represent each paper's contributions.

3. **Information Not Present/Unsubstantiated**: Remove or modify claims that lack support in the cited sources.

4. **Misrepresentation/Imprecise Wording**: Refine the wording to accurately capture the nuance and emphasis of the original sources.

For each error, provide:
1. **Corrected Claim**: A revised version that accurately reflects the cited sources
2. **Explanation**: Brief explanation of what was changed and why
3. **Key Changes**: Bullet points highlighting the main modifications
...
Please process the following citation errors and provide corrections,the refined claim should be consistent with the source and keep precise:
...
def process_data_for_related_work_prompt_for_naive_rag_gpt(abstract:str,related_works:list[str]) -> str:
    prompt = """
    I am working on a research paper, and I need a well-written "Related Work" section. Below I'm providing you with the abstract of my paper and a list of abstracts of related works I've identified.

    Here's the abstract of the target paper:
    {abstract}
    
    Here's the list of abstracts of related works I've retrieved:
    """
    for i,related_work in enumerate(related_works):
        prompt += f"""
        {i+1}. {related_work["abstract"]}
        reference: {related_work["reference"]}
        """
    prompt += """
    
    Please write a "Related Work" section for my paper. The section should be well-written and comprehensive, covering all the related works in the list.
    Please exclusively respond with the "Related Work" section. Do not add any filler text before or after the "Related Work" section.
    Also, do not use any type of markdown formatting. 
    I want a pure text output only.
    
    **Output Format:**
    Each sentence with citation(s) should be like this: <cite>sentece_with_citation(s)</cite>
    ...
    {{
        ...
        "cite_ids": [
            ...
            {{"citation_text": "Wang et al.", "paper_id": "3"}}
        ]
    }}
    ...
    **Example of correct ordering:**    
    ...
    [
        {"citation_text": "Johnson et al. (2023)", "paper_id": "1"},
        {"citation_text": "Smith & Brown (2022)", "paper_id": "2"}, 
        {"citation_text": "Johnson et al. (2023)", "paper_id": "1"}
    ]
...
def process_data_for_related_work_prompt_for_baselines(abstract:str) -> str:
    
    prompt_section_1 = """
    Instruction:
    You will be given a target paper abstract. Your task is to write related work on a target paper.
    if you need to search the internet, please use the search tool. But you need to check if the search result is the same as the target paper. If it is, you should not use it.
    Output format:
    Output is related work that consists of 3~5 paragraphs. Ensure that there is a line break between each paragraph. 
    When citing references, use the format '[reference number]' for citations. Each citations should be in <cite></cite> tags.
    You must list all the references in the end.
    references typically include the authors, year, paper title, conference name, conference location/proceedings information, and sometimes page numbers and a DOI/URL or arxiv link.
    Example of reference:
    Tianlong Wang, Junzhe Chen, Xueting Han, Jing Bai (2024). CPL: Critical Plan Step Learning Boosts LLM Generalization in Reasoning Tasks. arXiv:2409.08642. https://arxiv.org/abs/2409.08642
...
def genrate_original_related_work_feedback_prompt(
    ...
    Please provide feedback on the original related work for revision.
    - structural logic, 
    - critical analysis, 
...
def generate_summary_prompt(abstract_source_paper: str, abstract_to_be_cited: str) -> str:
    ...
    """
def generate_related_work_revision_prompt_without_DAG(
    source_abstract: str,
    related_work: str,
    feedback: str,
) -> str:
    ...
    Also, do not use any type of markdown formatting. 
    * **Citation Format:** Each sentence with citation(s) should be like this: <cite>citation_text</cite>
    I want a pure text output only.
    """
    prompt_section_2 = """
    **Output Format:**
    ...
    {{
        ...
        "cite_ids": [
            {{"citation_text": "Smith et al., 2023", "paper_id": "1"}},
            {{"citation_text": "Jones & Brown, 2022", "paper_id": "2"}},
            {{"citation_text": "Wang (2024)", "paper_id": "3"}}
        ]
    }}
    """
    return prompt_section_1 + prompt_section_2
...
def generate_related_work_revision_prompt(
    ...
    The related work section is divided into {len(dimensions)} subsections:
    """
    ...
    for i in range(len(citations)):
        prompt_section_1 += f"""
        citation_info:
        - citation_text: {citations[i]['citation_text']}
        - paper_id: {citations[i]['paper_id']}
        """
    prompt_section_2 = """
    Output Format:
    You must return ONLY a valid JSON object with the following structure:
    ...
    **Important Notes for cite_ids:**
    ...
    - The "paper_id" should correspond to the paper_id from the input citation_info.
    - **CRITICAL: Record citations in the EXACT ORDER they appear in your related_work text**
    - If you cite the same paper multiple times, record each occurrence separately in the order they appear
    - Include ALL citations that appear in your related_work text
    - Be precise with author names and years as they appear in your citations
    ...
    **Example of correct ordering:**    
    ...
    [
        {"citation_text": "Johnson et al. (2023)", "paper_id": "1"},
        {"citation_text": "Smith & Brown (2022)", "paper_id": "2"}, 
        {"citation_text": "Johnson et al. (2023)", "paper_id": "3"}
    ]
...
def generate_related_work_prompt_with_arxiv_trees_post_hoc_refinement(
    source_abstract: str,
    dimensions: list[str],
    grouped: dict[str, list[dict]],
) -> str:
...
1. **Integration Strategy:** Carefully read the existing Related Work section and identify the most appropriate locations to integrate each new paper. The new content should flow naturally with the existing narrative.

2. **Structural Preservation:** Maintain the existing subsection structure and overall organization. Do not drastically reorganize the current content.

3. **Natural Integration:** 
   - Add new papers by expanding existing paragraphs where thematically appropriate
   - Create new paragraphs within existing subsections if needed
   - Ensure smooth transitions between existing and new content

4. **Citation Consistency:** 
   - Follow the exact same citation format as the existing work
   - Maintain chronological or logical citation order
   - Use <cite>citation_text</cite> format for all citations

5. **Content Enhancement:**
   - Draw connections between new papers and existing ones
   - Highlight how new papers complement, extend, or contrast with existing literature
   - Avoid redundancy with existing content
   - Keep the original content of the related work section
   - Keep precise and concise

6. **Coverage Requirements:**
   - All provided new papers must be integrated
   - Each new paper should be meaningfully discussed, not just mentioned
   - Maintain the academic tone and analytical depth of the original section

**Output Format:**
You must return ONLY a valid JSON object with the following structure:

{
    "enhanced_related_work": "Your complete enhanced related work section with new literature integrated...",
    "all_cite_ids": [
        {"citation_text": "Smith et al., 2023", "paper_id": "1"},
        {"citation_text": "Jones & Brown, 2022", "paper_id": "2"},
        {"citation_text": "New Author et al., 2024", "paper_id": "10"}
    ],
    "integration_summary": "Brief summary of where and how new papers were integrated..."
}

**Important Notes:**
- The "all_cite_ids" should include ALL citations (both existing and new) in the ORDER they appear in the enhanced text
- The "citation_text" must match EXACTLY how citations appear in the enhanced_related_work
- If existing citations change position due to integration, update their order accordingly
- Provide a brief integration_summary explaining your integration strategy

**Quality Criteria:**
- Seamless integration without awkward transitions
- Consistent academic tone throughout
- Logical flow and organization
- Complete coverage of all new papers
- Accurate and consistent citation formatting



I want a pure text output only in the format of JSON. Do not include any additional text, explanations, markdown or xml formatting outside of the JSON response. 
The citation_text should be the EXACT text you used in the related_work section (e.g., "Smith et al., 2023") and should be totally the same as the citation format in the related_work section.
"""
...
def generate_related_work_prompt_with_post_hoc_refinement(
    source_abstract: str,
    related_work: str,
    dimensions: list[str],
    grouped: dict[str, list[dict]],
    ...
    prompt_section_2 = f"""
    Below is the additional literature that I want to cite. 
    Because the given grouped related work is not enough in the aspect of Theoretical Foundation, Technical Approach, and Uncovered Sub-directions
    """
...
**Important Notes:**
- The "additional_cite_ids" should include ALL citations (both existing and new) in the ORDER they appear in the enhanced text
- The "citation_text" must match EXACTLY how citations appear in the enhanced_related_work
- If existing citations change position due to integration, update their order accordingly if necessary
...
**Output Format:**
    ...
    {{
        "enhanced_related_work": "Your complete related work section text here...",
        "additional_cite_ids": [
            {{"citation_text": "Smith et al., 2023", "paper_id": "1"}},
            {{"citation_text": "Jones & Brown, 2022", "paper_id": "2"}},
            {{"citation_text": "Wang (2024)", "paper_id": "3"}}
...
def generate_reflection_error_feedback_prompt(
    related_work: str,
    abstract: str,
) -> str:
    """
    Generates the reflection error feedback prompt for a given related work and error type.
    """
    output = f"""
    Related Work Literature Search Optimization Prompt Template  
    Instructions:  
    You are a professional research assistant tasked with analyzing the provided Related Work text to identify any information gaps. Follow these steps:  
    ...
    - **Domain Identification**: Identify the core research area.  
    - **Problem Classification**: Clarify whether the theoretical foundation is explained, whether the technical approach is complete, and whether there are any uncovered sub-directions.  
    ...
    {{  
        "Domain Identification": "Identify the core research area",  
        "Theoretical Foundation": "Missing theoretical foundations to be added, summarized in one sentence",  
        "Technical Approach": "Missing technical approaches to be added, summarized in one sentence",  
        "Uncovered Sub-directions": "Missing sub-directions to be added, summarized in one sentence"  
    }}  
    ```
    """
    return output

def generate_summary_prompt_with_second_round_retrieved_content(
    abstract_source_paper: str, abstract_to_be_cited: str, sentence_count: int = 8
) -> str:
    """
    ...
    Based on the two abstracts,write a brief few-sentence (at most {str(sentence_count)}) summary of the cited paper. Emphasize how the cited paper relates to my research.
